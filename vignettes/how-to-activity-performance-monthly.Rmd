...
To illustrate the basic workflow for accessing and visualising this publicly available data we'll look at three examples:

1. Measures describing different treatment end codes 
1. Measures describing wait times from referral to first treatment
1. Comparing a measure across different providers
...
We can also focus on a single measure and compare counts across different services. In this example we look at measure `M344` (_Count of referrals that ended in the reporting period with an end code of 'Termination of treatment earlier than Care Professional planned'_) and visualise trends over time for the four providers with the highest overall counts.

Here we identify the 4 providers with the most recorded activity in `M344` across the whole time period:
...
We can use this list (`top4_m344_providers`) to filter the data and plot trends for these four providers:

```{r fig-example3, warning=FALSE, message=FALSE, echo=FALSE, fig.height = 7, fig.width = 10, fig.cap = "Counts of referrals with measure M344 for the four providers with the highest totals."}
...
activity_performance |>
  ...
  filter(measure_id == "M344") |> 
  filter(group_type == "Provider") |> 
  filter(org_name2 %in% top4_m344_providers) |>
  mutate(
    org_name2 = str_to_title(org_name2),
    org_name2 = str_replace_all(org_name2, "Nhs", "NHS")
  ) |>
  ggplot(
    aes(
      y = value,
...
```{r review-spike-R1L-M344-2024-12, eval=FALSE, echo=FALSE}
# Code to investigate the spike in R1L for M344 in Dec 2024
# Need to do this properly to understand whats going on
raw_activity_performance <- nhstt:::read_raw(
  dataset = "activity_performance",
  period = "2024-12",
  frequency = "monthly",
  ) 

unique(raw_activity_performance$GROUP_TYPE)

unique(raw_activity_performance$ORG_NAME1)
unique(raw_activity_performance$ORG_CODE1)

length(unique(raw_activity_performance$ORG_NAME1))
# 119
length(unique(raw_activity_performance$ORG_CODE1))
# 120

unique(raw_activity_performance$ORG_NAME2)
unique(raw_activity_performance$ORG_CODE2)

length(unique(raw_activity_performance$ORG_NAME2))
# 141
length(unique(raw_activity_performance$ORG_CODE2))
# 141

raw_activity_performance |> 
  filter(GROUP_TYPE == "Provider") |>
  filter(ORG_NAME2 == "ESSEX PARTNERSHIP UNIVERSITY NHS FOUNDATION TRUST") |> 
  filter(MEASURE_ID == "M344") |> 
  pull(MEASURE_VALUE_SUPPRESSED) |> 
  as.numeric() |> 
  sum(na.rm = TRUE)
```