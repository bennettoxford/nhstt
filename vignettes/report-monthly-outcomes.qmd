...
# Helper function to prepare paired plot data (percentage + count)
prepare_paired_plot_data <- function(data, pct_measure, count_measure) {
  # Get percentage data
  pct_data <- data |>
    filter(measure_id == pct_measure) |>
    select(
      reporting_period,
      start_date,
      end_date,
    ...
    mutate(pct_measure_id = pct_measure)
...
# Prepare paired data for all three outcome measures (keep NAs for gap visualization)
plot_data_m186 <- prepare_paired_plot_data(ap, "M186", "M185")
plot_data_m192 <- prepare_paired_plot_data(ap, "M192", "M191")
plot_data_m195 <- prepare_paired_plot_data(ap, "M195", "M193")
