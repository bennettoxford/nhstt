...
# Metadata for the two treatment appointment measures
metadata_tx_appt <- get_metadata_monthly() |>
  filter(measure_id %in% c("M120", "M112")) |>
  select(measure_id, description)
```

::: {.callout-warning}

This report is in active development and we have not yet done extensive data assurance. This report demonstrates interactive features how we might want to present the data.

:::

This report explores two treatment appointment measures from the publicly available NHS Talking Therapies data. The search box allows you to search for and select up to three providers to highlight their scores over time You can also hover over the lines to explore the data interactively.

```{r}
#| label: tab-measures-tx-appt
#| echo: false
...
reactable(
  ...
  defaultPageSize = nrow(metadata_tx_appt),
  columns = list(
    measure_id = colDef(
      name = "Measure ID and description",
      cell = function(value, index) {
        description <- metadata_tx_appt$description[index]
        htmltools::tags$div(
          htmltools::tags$code(
...
# Prepare plot data for both measures (keep NAs for gap visualization)
plot_data_m120 <- prepare_plot_data(ap, "M120")

plot_data_m112 <- ap |>
  # There is something wrong with data from this service:
  # - in period 2024-06 they have 41
  # - in period 2025-07 they have -1.8
  filter(!org_code2 == "K7Y6R") |>
  prepare_plot_data("M112")

# Prepare organisation list for filtering (from M120 data)
org_list <- plot_data_m120 |>
  distinct(org_code2, org_name_code) |>
  arrange(org_code2)

# Pass data to Observable
ojs_define(plot_data_m120_ojs = plot_data_m120)
ojs_define(plot_data_m112_ojs = plot_data_m112)
...
// Helper function to convert R data to JavaScript
...
```{ojs}
//| label: url-parameter-handling

// Read URL parameter for organisation codes
urlOrgsParam = {
...
// Define color scale for selected organizations (3 distinct colors)
