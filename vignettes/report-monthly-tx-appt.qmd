...
# Metadata for the treatment appointment measures and reference measure
metadata_tx_appt <- get_metadata_monthly() |>
  filter(measure_id %in% c("M120", "M112", "M082")) |>
  select(measure_id, description)

# Helper function to prepare paired plot data (mean + count reference)
prepare_paired_plot_data <- function(data, mean_measure, count_measure) {
  # Get mean data
  mean_data <- data |>
    filter(measure_id == mean_measure) |>
    select(
      reporting_period,
      start_date,
      end_date,
    ...
    mutate(mean_measure_id = mean_measure)

  # Get count reference data
  count_data <- data |>
    filter(measure_id == count_measure) |>
    select(reporting_period, org_code2, value) |>
    rename(count_value = value) |>
    mutate(count_measure_id = count_measure)

  # Join and prepare
  mean_data |>
    left_join(count_data, by = c("reporting_period", "org_code2")) |>
...
# Prepare paired data for both appointment measures with M082 as reference
plot_data_m120 <- prepare_paired_plot_data(ap, "M120", "M082")
...
plot_data_m112 <- ap |>
  ...
  prepare_paired_plot_data("M112", "M082")
