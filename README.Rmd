...
tidy_sources <- nhstt:::load_tidy_sources_config()
...
df_nhstt_reports <- nhstt:::available_nhstt_reports() |>
  dplyr::filter(stringr::str_detect(title, "metadata")) |>
  dplyr::select(get_function, first_period, last_period, n_periods, version) |>
  dplyr::distinct() |>
  dplyr::left_join(
    dplyr::tibble(
      get_function = c(
  ...
  dplyr::left_join(tidy_url_lookup, by = "dataset") |>
  dplyr::mutate(get_function = paste0("`", get_function, "()`")) |>
  dplyr::select(get_function, first_period, last_period, n_periods, tidy_data, version)
